"""Inspect actual GUI-generated V2 extrusion paths; no printer connection."""
...
path=ROOT/'output/body-v2/kirby_body_v2_PLA_1h2m.gcode'
...
assert len(segments)==375,len(segments)
import zipfile,xml.etree.ElementTree as ET
with zipfile.ZipFile(ROOT/'output/body-v2/kirby-body-v2-X2D-black-PLA.3mf') as archive:
 ...
 centers=[positions[by_name[n]] for n in ('kirby_body_v2','retaining_nut_v2')]
...
for z,segs in segments.items():
 for c,r,label in [(centers[0],3.5,'body'),(centers[1],12.,'nut')]:
...
for ax,z in zip(axes.flat,selected):
 ...
 ax.set_xlim(centers[0][0]-27,centers[1][0]+22);ax.set_ylim(101,151);ax.set_aspect('equal')
 ax.set_title(f'Z = {z:.1f} mm');ax.set_xlabel('mm');ax.grid(alpha=.15)
fig.suptitle('V2 actual GUI slice / X2D / BLACK PLA / 0.20 mm / 375 layers',fontsize=15)
fig.text(.03,.01,'Both passages remain open. This is a geometry/toolpath check; physical fit, seal and print completion are not verified.',fontsize=10)
fig.tight_layout(rect=(0,.04,1,.94));fig.savefig(ROOT/'output/body-v2/layers.png',dpi=130);plt.close(fig)
report={'source':str(path.relative_to(ROOT)),'stage':'actual GUI slice','layers':len(segments),'selected_layers_mm':list(selected),'central_clear_core_violations':violations,'core_radii_mm':{'body':3.5,'nut':12.0},'extruding_arcs':arc_extrusions,'scope':'all-layer conservative core clearance plus six plotted XY layers; not full passage/strength verification','estimated_time':'1h1m54s','estimated_filament_g':25.91}
(ROOT/'design/body-v2/layer-inspection.json').write_text(json.dumps(report,indent=2)+'\n')
